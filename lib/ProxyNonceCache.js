...
class ProxyNonceCache {
    ...
    async #validateNonceAndIncrement(signerAddress, nonceType, nonceData, queryApi) {
        ...
        if (nonceIsExpired) {
            ...
            return (await this.cacheProvider.incrementNonce(signerAddress, nonceType, EXPIRY_UPDATE_ENUM.UpdateExpiry)).lastUsedNonce;
        }
    }
    ...
    async #refreshNonceFromChain(signerAddress, nonceType, nonceData, queryApi) {
        ...
        if (nonceData.lastUsedNonce === nonceFromChain) {
            ...
            const nonce = (await this.cacheProvider.incrementNonce(signerAddress, nonceType, EXPIRY_UPDATE_ENUM.DoNotUpade)).lastUsedNonce;
