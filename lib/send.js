...
function Send(api, queryApi, awtManager, signer) {
    console.log("Send constructor: ", api)
...
function transferAvt(api, queryApi, signer) {
  return async function (recipient, amount) {
    ...
    console.log("Transfer signer: ", signer);
...
Send.prototype.proxyRequest = async function (api, queryApi, apiSigner, methodArgs, transactionType, nonceType, retry) {
  ...
  if (nonceType !== NONCE_TYPE.None) {
    proxyArgs.nonce =
      nonceType === NONCE_TYPE.Nft
        ...
        : await api.nonceCache(queryApi).getNonceAndIncrement(apiSigner, nonceType);
  }
  ...
  if (api.hasSplitFeeToken() === false) {
    ...
    console.log("proxyRequest: ", apiSigner);
...
Send.prototype.getPaymentNonceAndSignature = async function (api, queryApi, signerAddress, paymentArgs, retry) {
    console.log("getPaymentNonceAndSignature: ", signerAddress);
  const { relayer, user, payer, proxySignature, transactionType } = paymentArgs;
  const paymentNonce = await api.nonceCache(queryApi).getNonceAndIncrement(payer, NONCE_TYPE.Payment);
