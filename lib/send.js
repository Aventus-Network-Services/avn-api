...
function Send(api, queryApi, awtManager, signer) {
...
function unstake(api, queryApi, signer) {
  return async function (unstakeAmount) {
    ...
    const accountInfo = await queryApi.getAccountInfo(signer);
...
Send.prototype.proxyRequest = async function (api, queryApi, apiSigner, methodArgs, transactionType, nonceType, retry) {
  ...
  let proxyArgs = Object.assign({ relayer, apiSigner, payer }, methodArgs);

  if (nonceType !== NONCE_TYPE.None) {
    proxyArgs.nonce =
      nonceType === NONCE_TYPE.Nft
        ? await queryApi.getNftNonce(methodArgs.nftId)
        : await this.smartNonce(queryApi, apiSigner, nonceType, retry);
  }
  ...
  if (api.hasSplitFeeToken() === false) {
    // By default the user pays the relayer fees but this can be changed to any `payer`
    const paymentArgs = { relayer, apiSigner, payer, proxySignature, transactionType };
...
Send.prototype.getPaymentNonceAndSignature = async function (api, queryApi, signer, paymentArgs, retry) {
  ...
  const feePaymentSignature = await proxyApi.generateFeePaymentSignature(feePaymentArgs, signer, api);
