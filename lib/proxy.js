...
const generateProxySignature = async (api, signerAddress, transactionType, proxyArgs) =>
...
async function signProxyTokenTransfer({ relayer, user, recipient, token, amount, nonce, signerAddress, api }) {
...
async function signProxyTokenLower({ relayer, user, token, amount, t1Recipient, nonce, signerAddress, api }) {
  relayer = common.convertToPublicKeyIfNeeded(relayer);
  user = common.convertToPublicKeyIfNeeded(user);
...
async function signProxyWithdrawUnlocked({ relayer, user, nonce, signerAddress, api }) {
  relayer = common.convertToPublicKeyIfNeeded(relayer);
...
async function signProxyExecuteLeaveNominators({ relayer, user, nonce, signerAddress, api }) {
  const dataRelayer = common.convertToPublicKeyIfNeeded(relayer);
...
async function generateFeePaymentSignature({ relayer, user, proxySignature, relayerFee, paymentNonce }, signerAddress, api) {
  relayer = common.convertToPublicKeyIfNeeded(relayer);
  user = common.convertToPublicKeyIfNeeded(user);
...
async function signData(api, signerAddress, encodedDataToSign) {
