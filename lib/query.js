...
function Query(api) {
  ...
  this.getStakerRewardsEarned = generateFunction(getStakerRewardsEarned, api);
...
function getNftNonce(api) {
  return async function (nftId) {
    nftId = common.formatNftId(nftId);
...
function getStakerRewardsEarned(api) {
  return async function (accountId, fromTimestamp = null, toTimestamp = null) {
    common.validateAccount(accountId);

    return await this.postRequest(api, 'getStakerRewardsEarned', { accountId, fromTimestamp, toTimestamp });
  };
}
