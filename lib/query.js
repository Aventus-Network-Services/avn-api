...
Query.prototype.postRequest = async function (api, signer, method, params, handler = 'query') {
  ...
  const response = await api.axios(this.awtManager.getToken()).post(endpoint, { jsonrpc: '2.0', id: api.uuid(), method: method, params: params });
...
Query.prototype.getRequest = async function (api, signer, params, handler = 'query') {
  ...
  const response = await api.axios(this.awtManager.getToken()).get(endpoint);
